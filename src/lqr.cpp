...
// lqr method (continuous time) LQR Solver for Continuous Time Infinite Horizon
// Problem Returns true if successful and outputs calculated gain matrix in
// input pointer
...
void balance_matrix(const Eigen::MatrixXd& A, Eigen::MatrixXd& Aprime,
                    Eigen::MatrixXd& D);
...
typedef long int logical;  // logical is the return type of select function to
                           // be passed to dgees_
typedef logical(selectFcn)(
    double*, double*);  // function pointer type for select function
logical select(double* wr,
               double* wi) {  // the function to be passed as select function
  return (*wr < 0);
}
...
extern "C" void dgees_(const char* JOBVS, const char* SORT, selectFcn* SELECT,
                       const int* N, double* A, const int* LDA, int* SDIM,
                       double* WR, double* WI, double* VS, const int* LDVS,
                       double* WORK, const int* LWORK, logical* BWORK,
                       int* INFO);
...
// Eigen::MatrixXd& K) { Eigen::MatrixXd n = Eigen::MatrixXd::Zero(a.rows(),
// b.cols()); return lqr(a, b, q, r, n, k);
// Check n
// if (N.rows() != B.rows() || N.cols() != B.cols()) {
//    cout << "LQR Error: Fifth (N) matrix must be identically dimensioned with
//    second (B) matrix!" << endl; return false;
//}
//}
...
bool lqr(const Eigen::MatrixXd& A, const Eigen::MatrixXd& B,
  ...
  // Check b
  if (B.rows() != A.cols()) {
    cout << "LQR Error: First (A) and second (B) matrix must be conformal!"
         << endl;
    return false;
  }
  ...
  if (Q.rows() != Q.cols() || Q.rows() != A.cols()) {
    cout << "LQR Error: Third (Q) matrix must be square and conformal with the "
            "first (A) matrix!"
         << endl;
    return false;
  }
  ...
  if (R.rows() != R.cols() || R.cols() != B.cols()) {
    cout << "LQR Error: Fourth (R) matrix must be square and conformal with "
            "column dimension of second (B) matrix!"
         << endl;
    return false;
  }
  ...
  Eigen::VectorXd D(n);
  Eigen::VectorXd s(n2);
  Eigen::MatrixXd X1(n, n), X2(n, n), L(n, n), U(n, n), Xa(n, n), Xb(n, n),
      X(n, n);
  ...
  double WR[n2], WI[n2];  // Real and imaginary parts of the computed
                          // eigenvalues in the same order that they appear on
                          // the diagonal of the output Schur form T
  Eigen::MatrixXd Z(n2,
                    n2);  // Contains the orthogonal matrix Z of Schur vectors

  // The fixed part of the scratch space for dgees_ to do calculations
  logical BWORK[n2];

  // Determine the optimal work size (size of the variable-sized scratch space
  // for dgees_ to do its calculations)
  double WORKDUMMY;
  int LWORK = -1;
  int INFO = 0;
  dgees_("V", "S", select, &n2, H.data(), &LDA, &SDIM, &WR[0], &WI[0], Z.data(),
         &LDVS, &WORKDUMMY, &LWORK, &BWORK[0], &INFO);
  LWORK = int(WORKDUMMY) + 32;
  Eigen::VectorXd WORK(LWORK);

  // dgees call for real schur decomposition along with ordering
  dgees_("V", "S", select, &n2, H.data(), &LDA, &SDIM, &WR[0], &WI[0], Z.data(),
         &LDVS, WORK.data(), &LWORK, &BWORK[0], &INFO);
