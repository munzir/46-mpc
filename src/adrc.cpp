...
void computeLinearizedDynamics(const SkeletonPtr robot, Eigen::MatrixXd& A,
  ...
  Eigen::Matrix<double, 4, 4> baseTf =
      robot->getBodyNode(0)->getTransform().matrix();
  ...
  COM = Rot0 * (getBodyCOM(robot) - xyz0);
  COM(1) = 0;
  l_g = COM.norm();
  ...
  c1 = (M_g + m_w) * pow(r_w, 2) + I_wa + I_ra * pow(gamma, 2) +
       M_g * r_w * l_g + I_ra * pow(gamma, 2);
  c2 = M_g * r_w * l_g + M_g * pow(l_g, 2) + I_yy;

  // ******************** Robot dynamics for LQR Gains
  // A << 0, 0, 1, 0, //thCOM
  //      0, 0, 0, 1, //thWheel
  //      ((M_g+m_w)*pow(r_w,2)+I_wa+I_ra*pow(gamma,2))*M_g*g*l_g/delta, 0,
  //      -c1*c_w/delta, c1*c_w/delta, //dthCOM
  //      (M_g*r_w*l_g-I_ra*pow(gamma,2))*M_g*g*l_g/delta, 0, c2*c_w/delta,
  //      -c2*c_w/delta; //dthWheel
  ...
  A << 0, 1, 0, 0,  // thCOM
      ((M_g + m_w) * pow(r_w, 2) + I_wa + I_ra * pow(gamma, 2)) * M_g * g *
          l_g / delta,
      -c1 * c_w / delta, 0, c1 * c_w / delta,  // dthCOM
      0, 0, 0, 1,                              // thWheel
      (M_g * r_w * l_g - I_ra * pow(gamma, 2)) * M_g * g * l_g / delta,
      c2 * c_w / delta, 0, -c2 * c_w / delta;  // dthWheel
  ...
  // ********************** Observer Dynamics
  B_thWheel << 0, c2 / delta, 0;
  B_thCOM << 0, -c1 / delta, 0;
}
