...
class DDP {
 ...
 public:
  ...
  DDP(Scalar time_step, int time_steps, int iterations, util::Logger *logger,
      bool verbose,
      ...
      : dt_(time_step),
        ...
        logger_(logger),
        verbose_(verbose),
        Lk_(H_, Eigen::Matrix<Scalar, DynamicsT::ControlSize,
                              DynamicsT::StateSize>::Zero()),
        df_(H_, Eigen::Matrix<Scalar, DynamicsT::StateSize,
                              DynamicsT::StateSize +
                                  DynamicsT::ControlSize>::Zero()),
        d2L_(H_, Eigen::Matrix<
                     Scalar, DynamicsT::StateSize + DynamicsT::ControlSize,
                     DynamicsT::StateSize + DynamicsT::ControlSize>::Zero()),
        Vxx_(H_, Eigen::Matrix<Scalar, DynamicsT::StateSize,
                               DynamicsT::StateSize>::Zero()),
        VxxT_(H_, Eigen::Matrix<Scalar, DynamicsT::StateSize,
                                DynamicsT::StateSize>::Zero()),
  ...
  OptimizerResult<DynamicsT> run(
      const Eigen::Ref<const State> &x0,
      const Eigen::Ref<const ControlTrajectory> &u, DynamicsT &dynamics,
    ...
    for (std::size_t i = 1; i < H_; ++i) {
      ...
      x_.col(i) =
          x_.col(i - 1) + dynamics.f(x_.col(i - 1), u_.col(i - 1)) * dt_;
    }
    ...
    while (current_iteration < iter_) {
      ...
      Vxx_.at(H_ - 1) =
          static_cast<Scalar>(0.5) * (Vxx_.at(H_ - 1) + VxxT_.at(H_ - 1));
        ...
        while (!accept) {
          ...
          for (std::size_t k = 0; k < H_ - 1; ++k) {
            ...
            cost_(current_iteration, k) =
                run_cost.c(xnew.col(k), unew.col(k)) * dt_;
          }
 ...
 private:
  ...
  util::EigenAlignedVector<Scalar, DynamicsT::ControlSize, DynamicsT::StateSize>
      Lk_;
  util::EigenAlignedVector<Scalar, DynamicsT::StateSize,
                           DynamicsT::StateSize + DynamicsT::ControlSize>
      df_;
  util::EigenAlignedVector<Scalar,
                           DynamicsT::StateSize + DynamicsT::ControlSize,
                           DynamicsT::StateSize + DynamicsT::ControlSize>
      d2L_;
  util::EigenAlignedVector<Scalar, DynamicsT::StateSize, DynamicsT::StateSize>
      Vxx_;
  util::EigenAlignedVector<Scalar, DynamicsT::StateSize, DynamicsT::StateSize>
      VxxT_;
  ...
  Eigen::Matrix<Scalar, DynamicsT::StateSize + DynamicsT::ControlSize,
                Eigen::Dynamic>
      dL_;
  ...
  bool warm_start_;
  double lambda_factor_, lambda_max_;
  double lambda_min_;
};
